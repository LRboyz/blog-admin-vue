    ...
    >
      ...
      <el-form-item label="配置角色" prop="role">
        <el-select v-model="form.role">
          <el-option v-for="(r, i) in roles" :key="i" :label="r.label" :value="r.value"></el-option>
        </el-select>
      </el-form-item>
        ...
        if (valid) {
          ...
          if (this.pageType === 'add') {
            try {
              ...
              this.loading = false
              this.$message.error(`${e.data.msg}` || '创建用户失败！')
              console.log(e)
            }
          } else {
            // 更新用户信息
            if (this.form.email === this.info.email && this.form.role === this.info.role) {
            ...
            try {
              this.loading = true
              res = await Admin.updateOneUser(this.form.email, this.form.role, this.id)
            } catch (e) {
              this.loading = false
              console.log(e)
            }
